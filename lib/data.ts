 ...
 * id. Lets `getArticleById` resolve deep links (/?id=…) and search results
 * without another network round-trip.
 */
const seen = new Map<number, NewsArticle>();

function remember(items: NewsArticle[]) {
  for (const a of items) seen.set(a.id, a);
  // Persist snapshots so liked items + deep links survive a reload / offline.
  // Imported lazily to avoid pulling IndexedDB into the server bundle.
  if (typeof window !== "undefined") {
    import("./db")
      .then((m) => m.cacheArticles(items))
      .catch(() => {});
  }
}

/**
 * Fetch a page of live news from the RSS aggregator API route. Falls back to
 * the bundled sample articles if the network/API is unavailable, so the app
...
export async function fetchNewsPage(
  ...
  return fetchSamplePage(page, category);
}

/** Offline/fallback pagination over the bundled sample articles. */
async function fetchSamplePage(
  page: number,
  category: NewsCategory,
): Promise<{ items: NewsArticle[]; hasMore: boolean }> {
  await new Promise((r) => setTimeout(r, page === 0 ? 250 : 450));
  ...
  for (let i = 0; i < PAGE_SIZE; i++) {
    ...
    items.push(loop === 0 ? base : { ...base, id: base.id + loop * 1000 });
  }
  remember(items);
...
/**
 * Resolve an article by id: checks this session's cache, then the IndexedDB
 * snapshot store, then the bundled sample data.
 */
export async function getArticleById(
  ...
  if (typeof window !== "undefined") {
    try {
      const { getCachedArticle } = await import("./db");
