...
/** Bulk-cache article snapshots (e.g. everything loaded into the feed). */
export async function cacheArticles(articles: NewsArticle[]): Promise<void> {
  if (articles.length === 0) return;
  const db = await getDB();
  const tx = db.transaction("articles", "readwrite");
  await Promise.all([
    ...articles.map((a) => tx.store.put(a)),
    tx.done,
  ]);
}

/** Look up a cached article snapshot by id. */
